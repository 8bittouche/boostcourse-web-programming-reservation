...
<body>
    <div id="container">
            ...
            <header class="header_tit">
                ...
                <c:choose>
                	<c:when test="${sessionScope.reservationEmail != null}">
                		<a href="myreservation?reservationEmail=${sessionScope.reservationEmail }" class="btn_my"> <span class="viewReservation" title="예약확인">${sessionScope.reservationEmail }</span> </a>
                	</c:when>
                	<c:otherwise>
        ...
        <div class="ct">
            <div class="ct_wrap">
            	<input type="hidden" id="displayInfoId" value=${param.id } />
                <div class="top_title">
                    <a href="detail?id=${param.id }" class="btn_back" title="이전 화면으로 이동"> <i class="fn fn-backward1"></i> </a>
                    <h2><span class="title"></span></h2>
                </div>
                <div class="group_visual">
                    <div class="container_visual" style="height: 414px">
                        <ul class="visual_img" style="height: 100%">
                        </ul>
                    </div>
                </div>
                <div class="section_store_details">
                    <div class="store_details">
						<h3 class="in_tit">장소</h3>
							<p class="dsc placeName">
							</p>
						<h3 class="in_tit">관람시간</h3>
							<p class="dsc openingHours">
							</p>
                            ...
                            <form class="form_horizontal">
                                <div class="inline_form"> <label class="label" for="name"> <span class="spr_book ico_nessasary">필수</span> <span>예매자</span> </label>
                                    <div class="inline_control"> 
                                    	...
                                    	<div class="invalid_msg" id="name_invalid_msg"><span>예매자 이름을 입력해주세요</span></div>
                                    </div>
                                </div>
                                <div class="inline_form"> <label class="label" for="tel"> <span class="spr_book ico_nessasary">필수</span> <span>연락처</span> </label>
                                    <div class="inline_control tel_wrap">
                                        ...
                                        <div class="invalid_msg" id="tel_invalid_msg"><span>형식이 틀렸거나 너무 짧아요</span></div>
                                    </div>
                                </div>
                                <div class="inline_form"> <label class="label" for="email">  <span class="spr_book ico_nessasary">필수</span>  <span>이메일</span> </label>
                                    <div class="inline_control"> 
                                    	...
                                    	<div class="invalid_msg email" id="email_invalid_msg"><span>형식이 틀렸거나 너무 짧아요</span></div> 
                                    </div>
                                </div>
                                <div class="inline_form last"> <label class="label" for="message">예매내용</label>
                                    <div class="inline_control">
                                        <p class="inline_txt selected">${requestScope.reservationDate}, 총 <span id="totalCount">0</span>매</p>
                                    </div>
                                </div>
                            </form>
                        </div>
...
<script type="text/x-handlebars-template" id="visual_img_item">
    <li class="item" style="width: 100%; height: 100%"> <img alt="{{saveFileName}}" class="img_thumb" src="download/{{saveFileName}}" > <span class="img_bg"></span>
    	<div class="preview_txt">
    		<h2 class="preview_txt_tit">{{productDescription}}</h2> <em class="preview_txt_dsc"></em><em class="preview_txt_dsc"></em> </div>
    </li>
</script>
<script type="text/x-handlebars-template" id="ticket_body_qty">
    <div class="qty">
		<div class="count_control">
			<div class="clearfix">
				<input type="hidden" value="{{priceType}}">
				<a class="btn_plus_minus spr_book2 ico_minus3 disabled" title="빼기"> </a> <input type="tel" id="count_control_input_{{priceType}}" class="count_control_input disabled" value="0" readonly title="수량">
				<a class="btn_plus_minus spr_book2 ico_plus3" title="더하기">
				</a>
			</div>
			<div class="individual_price"><span class="total_price"><span class="total_price_{{priceType}}">0</span></span><span class="price_type">원</span></div>
		</div>
		<div class="qty_info_icon"> <strong class="product_amount"> <span>{{priceTypeName}}</span> </strong> <strong class="product_price"> <span class="price"><span class="price_{{priceType}}">{{price}}</span></span> <span class="price_type">원</span> </strong> <em class="product_dsc">{{price}}원 ({{discountRate}}% 할인가)</em> </div>
	</div>
</script>
